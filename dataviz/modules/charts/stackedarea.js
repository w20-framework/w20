...
define([
    ...
    common.ngModule.directive('w20StackedAreaChart', [function () {
        return {
            restrict: 'A',
            scope: {
                config: '=w20StackedAreaChart'
            },
            template: '<svg></svg>',
                        ...
                        nv.addGraph({
                            generate: function () {
                                ...
                                if (scope.config.style) {
                                    chart.style(scope.config.style);
                                }

                                if (scope.config.order) {
                                    chart.order(scope.config.order);
                                }

                                if (scope.config.offset) {
                                    chart.offset(scope.config.offset);
                                }

                                if (scope.config.interpolate) {
                                    chart.interpolate(scope.config.interpolate);
                                }
                                ...
                                if (scope.config.xScale) {
                                    chart.xScale(scope.config.xScale());
                                }

                                if (scope.config.yScale) {
                                    chart.yScale(scope.config.yScale());
                                }

                                if (scope.config.xDomain) {
                                    chart.xDomain(scope.config.xDomain());
                                }

                                if (scope.config.yDomain) {
                                    chart.yDomain(scope.config.yDomain());
                                }

                                if (scope.config.sizeDomain) {
                                    chart.sizeDomain(scope.config.sizeDomain());
                                }
